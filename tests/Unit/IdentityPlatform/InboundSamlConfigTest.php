<?php

declare(strict_types=1);

namespace Kreait\Firebase\Tests\Unit\IdentityPlatform;

use Kreait\Firebase\Tests\UnitTestCase;
use Kreait\Firebase\Request\InboundSamlConfig;
use Kreait\Firebase\IdentityPlatform\IdpConfig;
use Kreait\Firebase\IdentityPlatform\SpConfig;

class InboundSamlConfigTest extends UnitTestCase
{
    public function testWithProperties()
    {
        $spConfig = [
                'spEntityId' => 'testSp',
                'callbackUri' => 'https://google.com',
            ];

        $idpConfig =  [
            'idpEntityId' => 'https://google.com',
            'ssoUrl' => "https://google.com",
            'idpCertificates' => [['x509Certificate' => '-----BEGIN CERTIFICATE-----

c7tL1QjbfAUHAQYwmHkWgPP+T2wAv0pOt36GgMCM
-----END CERTIFICATE-----
',
                ]],
            'signRequest' => true
        ];

        $properties = [
            'name' => 'saml.test',
            'idpConfig' => $idpConfig,
            'spConfig' => $spConfig,

        ];

        $propertiesConverted = [
            'name' => 'saml.test',
            'idpConfig' => IdpConfig::withProperties($idpConfig),
            'spConfig' => SpConfig::withProperties($spConfig),

        ];
        $propertiesConvertedwithNulls = [
            'name' => 'saml.test',
            'enabled' => null,
            'displayName' => null,
            'idpConfig' => IdpConfig::withProperties($idpConfig),
            'spConfig' => SpConfig::withProperties($spConfig),

        ];



        $instance = InboundSamlConfig::withProperties($properties);

        $this->assertEquals($propertiesConvertedwithNulls, $instance->toArray());
        $this->assertEquals($propertiesConverted, $instance->jsonSerialize());
    }
}
