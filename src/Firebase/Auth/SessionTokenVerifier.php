...
final class SessionTokenVerifier implements TokenVerifier
{
    const KEYS_URL = 'https://www.googleapis.com/identitytoolkit/v3/relyingparty/publicKeys';
    const ISSUER_FORMAT = 'https://session.firebase.google.com/%s';
    ...
    private function getKey(Token $token): string
    {
        ...
        if (!($key = $keys[$keyId] ?? null)) {
            throw InvalidToken::because("The Token claims it has been signed by the key with ID {$keyId}, but Google testVerifyIdTokendoesn't know this key.");
        }
