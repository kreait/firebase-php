 ...
 * @internal
 */
final class ApiClientHandler implements Handler
{
    ...
    public function handle(SignIn $action): SignInResult
    {
        ...
        try {
            $response = $this->client->request($request, ['http_errors' => false]);
        } catch (Throwable $e) {
    ...
    private function anonymous(SignInAnonymously $action): RequestInterface
    {
        $body = Utils::streamFor(JSON::encode(self::prepareBody($action), JSON_FORCE_OBJECT));

        return $this->client->createProjectLessRequest('POST', '/accounts:signUp')
            ->withBody($body)
        ;
    }

    private function customToken(SignInWithCustomToken $action): RequestInterface
    {
        $body = Utils::streamFor(JSON::encode(\array_merge(self::prepareBody($action), [
            'token' => $action->customToken(),
        ]), JSON_FORCE_OBJECT));

        return $this->client->createProjectLessRequest('POST', '/accounts:signInWithCustomToken')
            ->withBody($body)
        ;
    }

    private function emailAndPassword(SignInWithEmailAndPassword $action): RequestInterface
    {
        $body = Utils::streamFor(JSON::encode(\array_merge(self::prepareBody($action), [
            'email' => $action->email(),
        ...
        return $this->client->createProjectLessRequest('POST', '/accounts:signInWithPassword')
            ->withBody($body)
        ;
    }

    private function emailAndOobCode(SignInWithEmailAndOobCode $action): RequestInterface
    {
        $body = Utils::streamFor(JSON::encode(\array_merge(self::prepareBody($action), [
            'email' => $action->email(),
        ...
        return $this->client->createProjectLessRequest('POST', '/accounts:signInWithEmailLink')
            ->withBody($body)
        ;
    }

    private function idpCredentials(SignInWithIdpCredentials $action): RequestInterface
    {
        $postBody = [
        ...
        return $this->client->createProjectLessRequest('POST', '/accounts:signInWithIdp')
            ->withBody($body)
        ;
    }

    private function refreshToken(SignInWithRefreshToken $action): RequestInterface
    {
        $body = Query::build([
